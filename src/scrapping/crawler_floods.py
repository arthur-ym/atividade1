...
from src.templates.dataclass import RawFlood, SilverFlood
        ...
        for day in all_days:
            ...
            for flood_ocurrence in flood_list:
                ...
                for flood_point in flood_points:
                    ...
                    logger.info('-' * 50)
                    logger.info(f'raw_flood: {raw_flood}')
                    logger.info('-' * 50)
    ...
    def _transformar(self):
        ...
        for i in range(len(self.df)):
            try:
                ...
                data_list.append(
                    SilverFlood(
                        data=self.df.loc[i, 'data'],
                        status=self.df.loc[i, 'status'],
                        periodo=self.df.loc[i, 'periodo'],
                        endereco=self.df.loc[i, 'endereco'],
                        sentido=self.df.loc[i, 'sentido'],
                        ...
                        endereco_modificado=endereco_modify,
                        referencia_modificado=referencia_modify,
        ...
        self.df = get_df_from_model_list(models=data_list)
